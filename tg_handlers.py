...
async def help_command(update: Update, context: CallbackContext):
    ...
    help_text = (
        ...
        "Setup:\n"
        "1. /setsender <your_email@example.com> - Set your sender email.\n By default, all files are sent from p@bushuyeu.\n\n"
        "Receivers:\n"
...
async def set_sender_command(update: Update, context: CallbackContext):
    ...
    if '@' not in email or '.' not in email.split('@')[-1]:
        ...
        await update.message.reply_text(f"{safe_email} is not a valid email.")
        return

    set_user_setting(user_id, 'sender_email', email)
    safe_email = email.replace('.', '\\.')
    await update.message.reply_text(f"Sender set to: {safe_email}")
    logger.info(f"User {user_id} set sender to {email}")

async def new_receiver_command(update: Update, context: CallbackContext):
    ...
    safe_email = email.replace('.', '\\.')
    await update.message.reply_text(f"Added {label}: {safe_email}")
    logger.info(f"User {user_id} added receiver {label}: {email}")

async def view_receivers_command(update: Update, context: CallbackContext):
    ...
    for label, email in sorted(receivers.items()):
        safe_email = email.replace('.', '\\.')
        lines.append(f"- {label}: {safe_email}")
    await update.message.reply_text("\n".join(lines))
    logger.info(f"User {user_id} viewed receivers")

async def remove_receiver_command(update: Update, context: CallbackContext):
    ...
    if label in receivers:
        ...
        set_user_setting(user_id, 'receivers', receivers)
        safe_email = email.replace('.', '\\.')
        await update.message.reply_text(f"Removed `{label}`: `{safe_email}`")
...
async def send_to_command(update: Update, context: CallbackContext):
    ...
    try:
        ...
        safe_recipient = recipient_email.replace('.', '\\.')
        await update.message.reply_text(f"Sending {safe_name} to {safe_recipient}...")
        ...
        safe_error = str(error).replace('_', '\\_')
